...
mkdir -p ${HOST_INSTALL_ROOT}/Python-${PYTHON3_VERSION}
...
docker run -it --rm \
  ...
  -v ${PWD}/Python-${PYTHON3_VERSION}-host:/home/nao/${PEPPER_INSTALL_ROOT}/Python-${PYTHON3_VERSION} \
...
docker run -it --rm \
  ...
  -v ${HOST_INSTALL_ROOT}/Python-${PYTHON3_VERSION}:/home/nao/${PEPPER_INSTALL_ROOT}/Python-${PYTHON3_VERSION} \
  -v ${ALDE_CTC_CROSS}:/home/nao/ctc \
  ros2-pepper \
  bash -c "\
    set -euf -o pipefail && \
